...
namespace Lab4
{
    public class SyncTasksHttpExecutor
    {
        ...
        public void Execute()
        {
            ...
            {
                // in spate este un thread pool
                _tasks.Add(Task.Factory.StartNew(Fetch, (_hosts[i], i)));
            }

            Task.WaitAll(_tasks.ToArray()); // wait for all requests to finish (task ~ thread)
        }
        ...
        private static void Fetch(object args)
        {
            ...
            ConnectWrapper(requestWrapper).Wait(); // Waits for the Task to complete execution (receive the signal)
            ...
            // release the socket
            clientSocket.Shutdown(SocketShutdown.Both); // Disable both sending and receiving on this Socket
            clientSocket.Close();
        }
        ...
        private static Task ConnectWrapper(RequestWrapper requestWrapper)
        {
            ...
            return Task.FromResult(requestWrapper.ConnectionFlag.WaitOne()); // wait for event signal
        }
        ...
        private static Task SendWrapper(RequestWrapper requestWrapper, string data)
        {
            ...
            // begin sending the request to the server  
        ...
        private static Task ReceiveWrapper(RequestWrapper requestWrapper)
        {
            ...
            requestWrapper.Socket.BeginReceive(requestWrapper.Buffer, 0, RequestWrapper.BufferSize, SocketFlags.None, ReceiveCallback, requestWrapper);
        ...
        private static void ReceiveCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // check if the data received so far has the length specified in the response headers
